...
Fheight's backend primarily consists of four CoffeeScript services:
...
- Client code can be found in `app` (see `new Firebase()` calls) and
	`server/lib/fheight_firebase_module.coffee`, and configuration can be found
	in `config/`
