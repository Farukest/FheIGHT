...
- [Open Issues](https://github.com/open-fheight/fheight/issues)
...
[Issue #4](https://github.com/open-fheight/fheight/issues/4)).
...
First, fork OpenFheight on Github, and push your branch to the fork. Then, when
...
branch. At this point, you are now an OpenFheight developer. Congratulations!

## Versioning <a id="versioning" />

OpenFheight uses [Semantic Versioning](https://semver.org/) for its releases.
In version `1.96.17`, `1` is the `MAJOR` version, `96` is the `MINOR` version,
and `17` is the `PATCH` version.

For OpenFheight, the `MAJOR` version should not exceed `1`. Note that the
immediate release after `1.99` is `1.100` and not `2.0.0`.

## Where to Get Help <a id="help" />

At the moment, you can get help with OpenFheight by opening an issue. Since this
is a volunteer project, it may take a while for someone to look at your issue.

You can also join the [OpenFheight Discord server](https://discord.gg/HhUWfZ9cxe)
