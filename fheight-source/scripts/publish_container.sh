...
LOCAL_IMAGE="fheight-${SERVICE}:${VERSION}"
REMOTE_IMAGE="public.ecr.aws/${REGISTRY}/fheight-${SERVICE}:${VERSION}"
...
REMOTE_IMAGE="public.ecr.aws/${REGISTRY}/fheight-${SERVICE}:latest"
echo "Tagging $LOCAL_IMAGE as $REMOTE_IMAGE"
docker tag $LOCAL_IMAGE $REMOTE_IMAGE
echo "Pushing $REMOTE_IMAGE"
docker push $REMOTE_IMAGE
