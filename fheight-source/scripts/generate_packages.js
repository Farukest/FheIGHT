...
(function () {
  ...
  const Cards = require('app/sdk/cards/cardsLookupComplete.js');
  const FactionsLookup = require('app/sdk/cards/factionsLookup.js');
  const FactionFactory = require('app/sdk/cards/factionFactory.js');
  const CodexChapters = require('app/sdk/codex/codexChapterLookup.js');
  const CosmeticsLookup = require('app/sdk/cosmetics/cosmeticsLookup.js');
  ...
  const parseFactionFactory = function (file, content) {
    ...
    for (let i = 1, il = factionBlocks.length; i < il; i++) {
      ...
      const factionAliasMatch = factionBlock.match(/id:[\s\t]*?Factions\.(\w+)[,\r\n]/);
      if (!factionAliasMatch) continue;
      const factionAlias = factionAliasMatch[1];
  ...
  Promise.all([
    helpers.readFile(`${dir}/../app/application.js`, mapResourcesForFile),
    ...
    helpers.readFile(`${dir}/../dist/src/fheight.css`, mapResourcesForFile),
    ...
    return Promise.all([
      helpers.readFile(`${dir}/../app/sdk/cards/factionFactory.js`, parseFactionFactory),
      helpers.readFile(`${dir}/../app/sdk/codex/codex.js`, parseCodex),
    ...
    // and card skin packages can be correctly generated
      helpers.readFile(`${dir}/../app/sdk/cosmetics/cosmeticsFactory.js`, parseCosmeticsFactory))
    .then(() => {
