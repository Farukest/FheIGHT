...
var NewsManager = Manager.extend({
  ...
  onBeforeConnect: function () {
    ...
    ProfileManager.getInstance().onReady()
      ...
      .then(function () {
        ...
        this.newsItemsIndexCollection = new FHEIGHTFirebase.Collection(null, {
          firebase: new Firebase(process.env.FIREBASE_URL).child('news').child('index').limitToLast(10),
        });

        this.readNewsItemsCollection = new FHEIGHTFirebase.Collection(null, {
  ...
  getFirstUnreadAnnouncementContentAsync: function (callback) {
    ...
    if (announcement) {
      ...
      var content = new FHEIGHTFirebase.Model(null, {
        firebase: fbRef,
      });
