  ...
  onBeforeConnect: function () {
    ...
    ProfileManager.getInstance().onReady()
      ...
      .then(function () {
        ...
        this.topRankingModel = new FHEIGHTFirebase.Model(null, { firebase: process.env.FIREBASE_URL + '/user-ranking/' + userId + '/top' });

        // initialize a model for the users stats
        this.userStatsModel = new FHEIGHTFirebase.Model(null, { firebase: process.env.FIREBASE_URL + '/user-stats/' + userId });

        // init player games collection
        this.playerGames = new FHEIGHTFirebase.Collection(null, { firebase: new Firebase(process.env.FIREBASE_URL + '/user-games/' + userId).limit(1) });

        // init game invites collection
        this.receivedInvitesCollection = new FHEIGHTFirebase.Collection(null, { firebase: process.env.FIREBASE_URL + '/matchmaking/' + process.env.NODE_ENV + '/invites/to/' + userId });
        this.listenTo(this.receivedInvitesCollection, 'add', this._onGameInviteReceived);

        // initialize a rank model
        this.rankingModel = new FHEIGHTFirebase.Model(null, { firebase: process.env.FIREBASE_URL + '/user-ranking/' + userId + '/current' });
        this.rankingModel.onSyncOrReady()
          ...
          .then(function (response) {
            ...
            this.historyRankingModelCollection = new FHEIGHTBackbone.Collection();
            ...
            this.ladderPositionModel = new FHEIGHTBackbone.Model();
  ...
  */
  ...
  invitePlayerToGame: function (playerId, playerName) {
    if (!this.lastSentInviteModel || this.lastSentInviteModel.get('toId') !== playerId) {
      ...
      this.lastSentInviteModel = new FHEIGHTFirebase.Model(inviteData, { firebase: inviteRef });
