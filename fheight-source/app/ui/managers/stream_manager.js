...
var StreamManager = Manager.extend({
  ...
  constructor: function () {
    this.liveStreamCollection = new FHEIGHTBackbone.Collection();
  ...
  onBeforeConnect: function () {
    ...
    this.streamerWhitelistCollection = new FHEIGHTFirebase.Collection(null, {
  ...
  loadStreamStatusFromTwitch: function () {
    ...
    return Promise.all(loadPromises).then(function (results) {
      for (var i = 0; i < results.length; i++) {
        if (results[i] && results[i].stream && results[i].stream.game.toLowerCase() === 'fheight') {
          ...
          this.liveStreamCollection.push({
            ...
            description: model.get('description'), // 'Noah streams FHEIGHT daily. Subscribe and submit your decks to the Deck Doctor.',
