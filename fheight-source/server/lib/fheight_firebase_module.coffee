...
class FHEIGHTFirebaseModule
  ...
  @connect: (firebaseUrl = defaultFirebaseUrl) ->
    ...
    @apps[key] = new FHEIGHTFirebaseModule(
      key: key,
  ...
  @disconnect: (url) ->
    if @apps[url]?
      ...
      delete FHEIGHTFirebaseModule.apps[url]
    else
  ...
  constructor: ({@key, @firebaseUrl}) ->
    ...
    @promise.catch (error) =>
      delete FHEIGHTFirebaseModule.apps[@key]
...
module.exports = FHEIGHTFirebaseModule
