...
module "ecs_service_api" {
  ...
  secrets = [
    { name = "FIREBASE_LEGACY_TOKEN", valueFrom = "/fheight/staging/firebase/legacy-token" },
    { name = "FIREBASE_CLIENT_EMAIL", valueFrom = "/fheight/staging/firebase/client-email" },
    { name = "FIREBASE_PRIVATE_KEY", valueFrom = "/fheight/staging/firebase/private-key" },
    { name = "POSTGRES_CONNECTION", valueFrom = "/fheight/staging/postgres/connection-string" }
  ]
}

module "ecs_service_game" {
  source            = "../modules/ecs_service"
  name              = "fheight-game-staging"
  cluster           = module.ecs_cluster.id
  capacity_provider = module.ecs_cluster.spot_capacity_provider
  task_role         = module.ecs_cluster.task_role
