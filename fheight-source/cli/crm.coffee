...
program
  ...
  .action (args,callback)->
    ...
    listName = "FHEIGHT BETA Requests"
...
.action (args,callback)->
  ...
  .then () ->
    ...
    .then ()->
      ...
      return UsersModule.inGameNotify(@.userId,"Your Spirit Orbs have been updated, please restart Fheight to avoid any issues","crm")
    callback()
  .catch (e)->
    console.log prettyError.render(e)
    callback(e)
...
.action (args,callback)->
  ...
  .then ()->
    return UsersModule.inGameNotify(@.userRow.id,"Your Player Progression has been updated, please restart Fheight to avoid any issues","crm")
  .then ()->
    callback()
  .catch (e)->
    console.log prettyError.render(e)
...
.action (args,callback)->
  ...
  .then ()->
    return UsersModule.inGameNotify(@.userRow.id,"Your Faction Progression has been updated, please restart Fheight to avoid any issues","crm")
  .then ()->
    callback()
  .catch (e)->
    console.log prettyError.render(e)
...
.action (args,callback)->
  ...
  .then (achievementRows)->
    ...
    return Promise.all(allPromises)
  .then () ->
    return UsersModule.inGameNotify(@.userRow.id,"Your Achievement Progress has been updated, please restart Fheight","crm")
  .then ()->
            ...
            .then (answer)->
              ...
              .then ()->
                if not mailer?
                  ...
                  Promise.promisifyAll(mailer)
                console.log "Email sent to #{@.email}".green
                return mailer.sendMailAsync(@.username,@.email,"FHEIGHT Purchase #{errorRow.transaction_id} Resolved","Your FHEIGHT purchase was processed and we've awarded you #{bonus_gold} BONUS GOLD for the delay and inconvenience. We apologize for the delay, but your transaction was marked as UNVERIFIED so it required a manual check on our end. Please contact support if you have any further issue or questions.")
...
.action (args,callback)->
  ...
  .then ()->
    ...
    return Promise.all([
      FheightFirebase.connect().getRootRef(),
      FheightFirebase.connect(stagingURL,stagingToken).getRootRef()
    ])
