...
describe('rank module', function () {
  ...
  before(function () {
    ...
    return UsersModule.createNewUser('unit-test@fheight.local', 'unittest', 'hash', 'kumite14')
      .then((userIdCreated) => {
        Logger.module('UNITTEST').log('created user ', userIdCreated);
        userId = userIdCreated;
      }).catch(Errors.AlreadyExistsError, (error) => {
        Logger.module('UNITTEST').log('existing user');
        return UsersModule.userIdForEmail('unit-test@fheight.local').then((userIdExisting) => {
          Logger.module('UNITTEST').log('existing user retrieved', userIdExisting);
  ...
  describe('cycleUserSeasonRanking()', () => {
    ...
    it('expect rank data to be correctly set in DB and Firebase', () => FheightFirebase.connect().getRootRef()
      .bind({})
    ...
    it(`expect the most recent rank in the DB and Firebase history to exist and to be unread and for the ${moment().utc().format('MMMM YYYY')} season`, () => FheightFirebase.connect().getRootRef()
      .bind({})
      .then((rootRef) => Promise.all([
    ...
    it(`expect the top rank record in the DB and Firebase to exist and to be unread and for the ${moment().utc().format('MMMM YYYY')} season`, () => FheightFirebase.connect().getRootRef()
      .bind({})
      .then((rootRef) => Promise.all([
        knex.first().from('users').where('id', userId),
  ...
  describe('cycleUserSeasonRanking() srank', () => {
    ...
    it('expect players to have accurate ladder position and rating after cycling if (S-Rank and Non-Srank)', () => {
      ...
      // set up users
      return Promise.all([
        createOrWipeUser('unit-test-rating-1@fheight.local', player1UserName, 0, startOfCurrentSeasonDate),
        createOrWipeUser('unit-test-rating-2@fheight.local', player2UserName, 0, startOfCurrentSeasonDate),
        createOrWipeUser('unit-test-rating-3@fheight.local', player3UserName, 5, startOfCurrentSeasonDate),
        createOrWipeUser('unit-test-rating-4@fheight.local', player4UserName, 2, startOfCurrentSeasonDate),
      ]).then(() => {
  ...
  describe('updateUserRankingWithGameOutcome()', () => {
    ...
    it('expect to update rank data with an additional win', () => {
      ...
      return RankModule.updateUserRankingWithGameOutcome(userId, true, gameId)
        .then((rankData) => {
          ...
          return FheightFirebase.connect().getRootRef()
            .bind({});
        }).then((rootRef) => Promise.all([
          knex.first().from('users').where('id', userId),
          FirebasePromises.once(rootRef.child('user-ranking').child(userId).child('current'), 'value'),
    ...
    it('expect rank (20) data to be correctly set in DB and Firebase', () => FheightFirebase.connect().getRootRef()
      .bind({})
      .then((rootRef) => Promise.all([
        knex.first().from('users').where('id', userId),
        FirebasePromises.once(rootRef.child('user-ranking').child(userId).child('current'), 'value'),
  ...
  describe('updateUsersRatingsWithGameOutcome()', () => {
    ...
    before(function () {
      this.timeout(10000);
      return Promise.all([
        createOrWipeUser('unit-test-rating-1@fheight.local', player1UserName, 20),
        createOrWipeUser('unit-test-rating-2@fheight.local', player2UserName, 20),
        createOrWipeUser('unit-test-rating-3@fheight.local', player3UserName, 0),
        createOrWipeUser('unit-test-rating-4@fheight.local', player4UserName, 0),
  ...
  describe('updateUsersRatingsWithGameOutcome() expected ladder Position', () => {
    ...
    before(function () {
      ...
      return Promise.all([
        createOrWipeUser('unit-test-position-1@fheight.local', player1UserName, 0),
        createOrWipeUser('unit-test-position-2@fheight.local', player2UserName, 0),
        createOrWipeUser('unit-test-position-3@fheight.local', player3UserName, 0),
        createOrWipeUser('unit-test-position-4@fheight.local', player4UserName, 0),
        createOrWipeUser('unit-test-position-5@fheight.local', player5UserName, 0),
        createOrWipeUser('unit-test-position-6@fheight.local', player6UserName, 0),
