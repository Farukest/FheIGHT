...
describe('migrations module', function () {
  ...
  before(function () {
    ...
    return Promise.all([
      createOrInsertUser('unit-test-1@fheight.local', 'player 1', 0),
      createOrInsertUser('unit-test-2@fheight.local', 'player 2', 0),
  ...
  describe('userCreateChargeCountsMigration()', () => {
    it('expect a player that did not purchase a starter bundle to have nothing in their firebase purchase count tree', () => SyncModule.wipeUserData(userId)
      ...
      })).then(() => MigrationsModule.userCreateChargeCountsMigration(userId)).then(() => FheightFirebase.connect().getRootRef())
