...
describe('users module', () => {
  ...
  describe('userIdForEmail()', () => {
    it('expect a user id if email exists', () => UsersModule.userIdForEmail('unit-test@fheight.local')
      .then((id) => {
        expect(id).to.exist;
  ...
  describe('createNewUser()', () => {
    ...
    describe('registration - when invite codes are active', () => {
      ...
      it('expect NOT to be able to create a user with an invalid invite code if invite codes are ACTIVE', () => {
        ...
        const email = `${rando}-unit-test@fheight.local`;
    ...
    describe('registration - with referral codes', () => {
      ...
      //   const email1 = rando1+'-unit-test@fheight.local'
      //   const username1 = rando1.toLowerCase()+'-unit-test'
      //   const rando2 = generatePushId()
      //   const email2 = rando2+'-unit-test@fheight.local'
  ...
  describe('updateUserProgressionWithGameOutcome() - codex reward', () => {
    before(() => FheightFirebase.connect().getRootRef()
      .bind({})
  ...
  describe('updateUserFactionProgressionWithGameOutcome()', () => {
    ...
    it('expect level 1 after 1 loss', () => FheightFirebase.connect().getRootRef()
      .bind({})
      .then((rootRef) => Promise.all([
  ...
  //   before(function(){
  //     Logger.module("UNITTEST").log("creating user");
  //     return UsersModule.createNewUser('unit-test-opponent@fheight.local','unittestopponent','hash','kumite14')
  //     .then(function(userIdCreated){
  //       Logger.module("UNITTEST").log("created user ",userIdCreated);
  //       opponentId = userIdCreated;
  //     }).catch(Errors.AlreadyExistsError,function(error){
  //       Logger.module("UNITTEST").log("existing user");
  //       return UsersModule.userIdForEmail('unit-test-opponent@fheight.local').then(function(userIdExisting){
  //         Logger.module("UNITTEST").log("existing user retrieved",userIdExisting);
