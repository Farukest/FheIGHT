...
describe('matchmaking', () => {
  ...
  before(function () {
    ...
    return UsersModule.createNewUser('unit-test@fheight.local', 'unittest', 'hash', 'kumite14')
      ...
      }).then(() => UsersModule.createNewUser('unit-test2@fheight.local', 'unittest', 'hash', 'kumite14'))
      .then((userIdCreated) => {
  ...
  describe('login player 1', () => {
    it('expect a player 1 token when logging in', function (done) {
      ...
      request
        ...
        .send({ email: 'unit-test@fheight.local', password: 'hash' })
        .expect(200)
        .end((err, res) => {
          expect(err).to.be.equal(null);
          expect(res.body).to.have.property('token');
          expect(jwt.decode(res.body.token).d.id).to.have.length(20);
          expect(jwt.decode(res.body.token).d.email).to.be.equal('unit-test@fheight.local');
  ...
  describe('login player 2', (done) => {
    it('expect a player 2 token when logging in', function (done) {
      ...
      request
        .post('/session')
        .set('Client-Version', version)
        .send({ email: 'unit-test2@fheight.local', password: 'hash' })
        .expect(200)
        .end((err, res) => {
          expect(err).to.be.equal(null);
          expect(res.body).to.have.property('token');
          expect(jwt.decode(res.body.token).d.id).to.have.length(20);
          expect(jwt.decode(res.body.token).d.email).to.be.equal('unit-test2@fheight.local');
